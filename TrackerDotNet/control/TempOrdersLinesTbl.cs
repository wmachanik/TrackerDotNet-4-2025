        ...
        public List<TempOrdersLinesTbl> GetAll(string SortBy)
        {
            ...
            if (dataReader != null)
            {
                while (dataReader.Read())
                    ...
                    {
                        TOLineID = dataReader["TOLineID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["TOLineID"]),
                        TOHeaderID = dataReader["TOHeaderID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["TOHeaderID"]),
                        ...
                        OriginalOrderID = dataReader["OriginalOrderID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["OriginalOrderID"])
                    });
                dataReader.Close();
            }
            trackerDb.Close();
