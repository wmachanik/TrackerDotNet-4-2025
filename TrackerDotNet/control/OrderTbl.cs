...
namespace TrackerDotNet.control
{
    ...
    public class OrderTbl
    {
        ...
        public long GetLastOrderAdded(long pCustomerID, DateTime pOrderDate, int pItemTypeID)
        {
        ...
        public OrderTblData GetOrderByID(long pOrderID)
        {
            ...
            if (dataReader != null)
            {
                if (dataReader.Read())
                {
                    ...
                    orderById.CustomerId = dataReader["CustomerID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["CustomerID"]);
