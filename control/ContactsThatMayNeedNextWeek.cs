...
public class ContactsThatMayNeedNextWeek
{
  ...
  public List<ContactsThayMayNeedData> GetContactsThatMayNeedNextWeek()
  {
    ...
    trackerDb.AddWhereParams((object) DateTime.Now.Date, DbType.Date);
    ...
    if (dataReader != null)
    {
      while (dataReader.Read())
        ...
        {
          CustomerData = {
            CustomerID = dataReader["ContactID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["ContactID"]),
          ...
          ClientUsageData = {
            LastCupCount = dataReader["LastCupCount"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["LastCupCount"]),
            NextCoffeeBy = dataReader["NextCoffeeBy"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextCoffeeBy"]).Date,
            NextCleanOn = dataReader["NextCleanOn"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextCleanOn"]).Date,
            NextFilterEst = dataReader["NextFilterEst"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextFilterEst"]).Date,
            NextDescaleEst = dataReader["NextDescaleEst"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextDescaleEst"]).Date,
            NextServiceEst = dataReader["NextServiceEst"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextServiceEst"]).Date,
          ...
          NextRoastDateByCityData = {
            ...
            NextPrepDate = dataReader["NextPreperationDate"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextPreperationDate"]).Date,
            NextDeliveryDate = dataReader["NextDeliveryDate"] == DBNull.Value ? DateTime.Now.Date : Convert.ToDateTime(dataReader["NextDeliveryDate"]).Date
          }
        });
      dataReader.Dispose();
    }
