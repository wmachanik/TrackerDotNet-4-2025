...
public class RepairsTbl
{
    ...
    if (dataReader != null)
    {
      while (dataReader.Read())
        allRepairs.Add(new RepairsTbl()
        {
          ...
          RelatedOrderID = dataReader["RelatedOrderID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["RelatedOrderID"]),
    ...
    if (pRepairID > 0)
    {
      ...
      if (dataReader != null)
      {
        if (dataReader.Read())
        {
          ...
          repairById.CustomerID = dataReader["CustomerID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["CustomerID"]);
          ...
          repairById.RelatedOrderID = dataReader["RelatedOrderID"] == DBNull.Value ? 0L : Convert.ToInt64(dataReader["RelatedOrderID"]);
  ...
  private bool LogARepair(RepairsTbl pRepair, bool pCalcOrderData)
  {
    ...
    if (pCalcOrderData)
    {
      ...
      pOrderData.ToBeDeliveredBy = contactPreferedItems.DeliveryByID;
    }
    else
    {
